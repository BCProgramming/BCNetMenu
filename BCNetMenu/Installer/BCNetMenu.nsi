...
!define FULLNAME "${COMPANY}\BASeCamp Network Menu"

!define INSTNAME "BASeCamp Network Menu (${PROGDATE}).exe"
...
Function InstComplete
  MessageBox MB_YESNO|MB_ICONQUESTION "Installation Complete$\r$\nStart BASeCamp Network Menu now?" /SD IDNO IDNO NoRun 
  Exec '"$INSTDIR\${PROGFILE}.exe"'
  NoRun:
FunctionEnd
Function .onInit
