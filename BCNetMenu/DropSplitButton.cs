...
{
    /// <summary>
    ///     A Drop Down split button similar to those used in Outlook.
    ///     This was originally added for Report Filter, but hopefully it can find use elsewhere.
    ///     Based on http://wyday.com/splitbutton/
    /// </summary>
    public class DropSplitButton : Button
    {
        private const int SplitterWidth = 17;
        private static readonly int UseBorderSize = SystemInformation.Border3DSize.Width*2;
        private PushButtonState _pushState;
        ...
        public Image DropDownImage { get { return _DropDownImage; } set { _DropDownImage = value; } }
        private TextFormatFlags textFormatFlags = TextFormatFlags.Default;

        public DropSplitButton()
        {
            AutoSize = true;
            /*System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(MfaListView));
            ImageList grabarrow = new ImageList();
            grabarrow.ImageStream = ((System.Windows.Forms.ImageListStreamer)(resources.GetObject("defaultImageList.ImageStream")));
            grabarrow.TransparentColor = Color.Transparent;
            grabarrow.Images.SetKeyName(0, "Blank.png");
            grabarrow.Images.SetKeyName(1, "UpArrow.png");
            grabarrow.Images.SetKeyName(2, "DownArrow.png");
            DropDownImage = grabarrow.Images["DownArrow.png"];*/
        ...
        /// <summary>
        ///     Get's/Sets the ContextMenuStrip associated with this control. This is rather important what with it
        ///     being sort of the entire point of the control.
        ///     Technically, this could be changed to support the various other Drop Downs available or a custom Drop Down similiar
        ///     to what the MfaComboBox does, but I didn't want to overcomplicate it- also, that would take
        ///     a while to build.
        /// </summary>
        [DefaultValue(null)]
        ...
        public ContextMenuStrip SplitMenuStrip
        {
            ...
            set
            {
                //remove handlers from old menustrip, then add the handlers for the new one.
        ...
        {
            get { return _pushState; }
            set
            {
                if (!_pushState.Equals(value))
                {
                    _pushState = value;
                    Invalidate();
                }
        ...
        protected override void OnGotFocus(EventArgs e)
        {
            ...
            if (!PushState.Equals(PushButtonState.Pressed) && !PushState.Equals(PushButtonState.Disabled))
            {
                PushState = PushButtonState.Default;
            }
        }
        ...
        protected override void OnKeyDown(KeyEventArgs kevent)
        {
            if (showSplit)
            {
                if (kevent.KeyCode.Equals(Keys.Down) && !isSplitMenuVisible) ShowContextMenuStrip();
                else if (kevent.KeyCode.Equals(Keys.Space) && kevent.Modifiers == Keys.None) PushState = PushButtonState.Pressed;
            }
        ...
        protected override void OnKeyUp(KeyEventArgs kevent)
        {
            if (kevent.KeyCode.Equals(Keys.Space))
            {
                ...
                {
                    PushState = PushButtonState.Normal;
                }
            }
        ...
        {
            PushState = Enabled ? PushButtonState.Normal : PushButtonState.Disabled;
        ...
        protected override void OnMouseLeave(EventArgs e)
        {
            ...
            if (!PushState.Equals(PushButtonState.Pressed) && !PushState.Equals(PushButtonState.Disabled))
            {
                PushState = Focused ? PushButtonState.Default : PushButtonState.Normal;
            }
        }
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            //when re-clicking the drop-down close the drop down.
            if (m_SplitMenu != null && e.Button == MouseButtons.Left && !isMouseEntered)
                skipNextOpen = true;

            if (dropDownRect.Contains(e.Location) && !isSplitMenuVisible && e.Button == MouseButtons.Left)
            {
                ShowContextMenuStrip();
            }
            else
            {
                PushState = PushButtonState.Pressed;
            }
        }
        ...
        protected override void OnMouseUp(MouseEventArgs mevent)
        {
            ...
            else if (m_SplitMenuStrip == null && m_SplitMenu == null || !isSplitMenuVisible)
            {
                ...
                if (ClientRectangle.Contains(mevent.Location) && !dropDownRect.Contains(mevent.Location))
                {
        ...
        protected override void OnPaint(PaintEventArgs pevent)
        {
            ...
            if (PushState != PushButtonState.Pressed && IsDefault && !Application.RenderWithVisualStyles)
            {
                Rectangle backgroundBounds = bounds;
                backgroundBounds.Inflate(-1, -1);
                ButtonRenderer.DrawButton(g, backgroundBounds, PushState);
            ...
            {
                ButtonRenderer.DrawButton(g, bounds, PushState);
            }

            // calculate the current dropdown rectangle.
            dropDownRect = new Rectangle(bounds.Right - SplitterWidth, 0, SplitterWidth, bounds.Height);

            int internalBorder = UseBorderSize;
            var focusRect =
                new Rectangle
                    (internalBorder - 1,
                        internalBorder - 1,
                        bounds.Width - dropDownRect.Width - internalBorder,
                        bounds.Height - (internalBorder*2) + 2);

            bool drawSplitLine = (PushState == PushButtonState.Hot || PushState == PushButtonState.Pressed || !Application.RenderWithVisualStyles);


            if (RightToLeft == RightToLeft.Yes)
            {
                dropDownRect.X = bounds.Left + 1;
                focusRect.X = dropDownRect.Right;

                if (drawSplitLine)
                {
                    // draw two lines at the edge of the dropdown button
                    g.DrawLine(SystemPens.ButtonShadow, bounds.Left + SplitterWidth, UseBorderSize, bounds.Left + SplitterWidth, bounds.Bottom - UseBorderSize);
                    g.DrawLine(SystemPens.ButtonFace, bounds.Left + SplitterWidth + 1, UseBorderSize, bounds.Left + SplitterWidth + 1, bounds.Bottom - UseBorderSize);
                }
            }
            else
            {
                ...
                {
                    // draw two lines at the edge of the dropdown button
                    g.DrawLine(SystemPens.ButtonShadow, bounds.Right - SplitterWidth, UseBorderSize, bounds.Right - SplitterWidth, bounds.Bottom - UseBorderSize);
                    g.DrawLine(SystemPens.ButtonFace, bounds.Right - SplitterWidth - 1, UseBorderSize, bounds.Right - SplitterWidth - 1, bounds.Bottom - UseBorderSize);
                }
            }

            // Draw an arrow in the correct location
            PaintArrow(g, dropDownRect);

            //paint the image and text in the "button" part of the splitButton
            PaintTextandImage(g, new Rectangle(0, 0, ClientRectangle.Width - SplitterWidth, ClientRectangle.Height));

            // draw the focus rectangle.
            if (PushState != PushButtonState.Pressed && Focused && ShowFocusCues)
            {
        ...
        private void PaintArrow(Graphics g, Rectangle dropDownRect)
        {
            ...
            {
                int pixelsize = Math.Min(dropDownRect.Width - 4, dropDownRect.Height - 4);
                //Size usesize = new Size(pixelsize,pixelsize);
                Rectangle useRect = new Rectangle(dropDownRect.Left + (int)(dropDownRect.Width / 2 - pixelsize / 2) - 2, dropDownRect.Top + (int)(dropDownRect.Height / 2 - pixelsize / 2), pixelsize, pixelsize);




                g.DrawImage(_DropDownImage, useRect, 0, 0, _DropDownImage.Width, _DropDownImage.Height, GraphicsUnit.Pixel);
            }
        ...
        public override Size GetPreferredSize(Size proposedSize)
        {
            ...
            if (showSplit)
            {
                ...
                if (!string.IsNullOrEmpty(Text) && TextRenderer.MeasureText(Text, Font).Width + SplitterWidth > preferredSize.Width)
                    return preferredSize + new Size(SplitterWidth + UseBorderSize*2, 0);
            }
        ...
        private void SplitMenuStrip_Closing(object sender, ToolStripDropDownClosingEventArgs e)
        {
            ...
            {
                skipNextOpen = (dropDownRect.Contains(PointToClient(Cursor.Position))) && MouseButtons == MouseButtons.Left;
            }
        }
        ...
        private void SetButtonDrawState()
        {
            ...
            {
                PushState = PushButtonState.Disabled;
            }
            else
            {
                PushState = PushButtonState.Normal;
        ...
        private void CalculateButtonTextAndImageLayout(ref Rectangle content_rect, out Rectangle textRectangle, out Rectangle imageRectangle)
        {
            ...
            switch (TextImageRelation)
            {
                case TextImageRelation.Overlay:
                    ...
                    if (_pushState == PushButtonState.Pressed && !Application.RenderWithVisualStyles)
        ...
        private void LayoutTextAboveOrBelowImage(Rectangle totalArea, bool textFirst, Size textSize, Size imageSize, out Rectangle textRect, out Rectangle imageRect)
        {
            ...
            if (textFirst)
            {
                ...
                final_image_rect = new Rectangle
                    (AlignInRectangle(totalArea, imageSize, ImageAlign).Left, final_text_rect.Bottom + element_spacing, imageSize.Width, imageSize.Height);
            }
            else
            {
